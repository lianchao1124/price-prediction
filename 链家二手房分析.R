...
# 数据探索
type1 <- ggplot(data = house, aes(alprice)) + geom_histogram(bins = 30)
type1

type1 <- ggplot(data = house, aes(area)) + geom_histogram(bins = 30)
type1

type2 <- ggplot(data = house, aes(floor)) + geom_bar()
type2

type3 <- ggplot(data = house, aes(angle)) + geom_bar()
type3

# 户型分布
type_freq <- data.frame(table(house$type))
type_p <- ggplot(data = type_freq, mapping = aes(x = reorder(Var1, -Freq),y = Freq)) + geom_bar(stat = 'identity', fill = 'steelblue') + theme(axis.text.x  = element_text(angle = 30, vjust = 0.5)) + xlab('type') + ylab('number')
type_p
...
source("D:/BaiduNetdiskDownload/二手房分析案例/normal.test.R")

# 面积的正态性检验
norm.test(house$area)

# 房价的正态性检验
norm.test(house$alprice)
...
# 上海各区房价均价
avg_price <- aggregate(house$price, by = list(house$area), mean)

ggplot(data = avg_price, mapping = aes(x = reorder(Group.1, -x), y = x, group = 1)) + geom_area(fill = 'lightgreen') + geom_line(colour = 'steelblue', size = 2) + geom_point() + xlab('') + ylab('mean price')

# 房屋建筑时间确实严重，我们按区域分组，使用众数填充
house$time[house$time == ''] <- NA
# 自定义众数函数
...
final_house <- subset(my.impute(house, 'area', 'time'),select = c(type.new,floow,area,alprice,price,time))
final_house <- transform(final_house, builtdate2now = 2016-as.integer(substring(as.character(time),1,4)))
final_house <- subset(final_house, select = -time)
...
standrad <- data.frame(scale(final_house[,c('area','alprice','price')]))
...
# 各户型的平均面积
aggregate(final_house$area, list(final_house$type.new), mean)

# 绘制面积与单价的散点图，并按聚类进行划分
p <- ggplot(data = final_house[,3:5], mapping = aes(x = area,y = price, color = factor(clust$cluster)))
...
fit2 <- lm(log(alprice) ~ .,data = model.data)
